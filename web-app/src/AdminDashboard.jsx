...
export default function AdminDashboard() {
    ...
    const fetchSystemData = async () => {
      ...
      `);
        ...
        // 3. Fetch pending stakeholder accounts awaiting approval
        const { data: pendingAccountData, error: pendingAccountError } = await supabase
            .from('profiles')
            .select('id, full_name, role, jurisdiction_state, created_at, is_approved')
            .eq('is_approved', false)
            .order('created_at', { ascending: false });

        if (!pendingAccountError && pendingAccountData) {
            setPendingAccounts(pendingAccountData);
        }
    ...
    const approveAccount = async (userId, userName) => {
        ...
        if (!error) {
            setPendingAccounts(prev => prev.filter(account => account.id !== userId));
            setMessage({ text: `${userName}'s account has been officially approved.`, type: 'success' });
            fetchSystemData(); // Refresh the lists
        } else {
    ...
    <div>
        ...
        {/* NEW: Stakeholder Approval Console */}
            ...
            <table className="modern-table">
                ...
                <tbody>
                    {pendingAccounts.length === 0 ? (
                        <tr>
                            <td colSpan="5" style={{ textAlign: 'center', color: '#6c757d' }}>No pending accounts to review.</td>
                        </tr>
                    ) : (
                        pendingAccounts.map((account) => (
                            <tr key={account.id}>
                                ...
                                <td>
                                    <span className="status-badge" style={{ background: '#e2e3e5' }}>
                                        {account.role.toUpperCase()}
                                    </span>
                                </td>
                                <td>{account.jurisdiction_state || 'Not specified'}</td>
                                <td>{new Date(account.created_at).toLocaleDateString()}</td>
                                <td>
                                    <button className="btn-success" onClick={() => approveAccount(account.id, account.full_name)}>
                                        Verify & Approve
                                    </button>
                                </td>
                            </tr>
